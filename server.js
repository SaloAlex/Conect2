...
app.use(
  cors({
    origin: "https://americas.api.riotgames.com", // Permitir el acceso desde cualquier origen
  })
);
...
app.get("/riot/account/v1/accounts/by-riot-id/:gameName/:tagLine", async (req, res) => {
  try {
    ...
    const response = await axios.get(`${baseURL}/riot/account/v1/accounts/by-riot-id/${encodeURIComponent(gameName)}/${encodeURIComponent(tagLine)}?api_key=${apiKey}`);
...
app.get("/lol/summoner/v4/summoners/by-name/:summonerName", async (req, res) => {
  try {
    ...
    const response = await axios.get(`${baseURL}/lol/summoner/v4/summoners/by-name/${encodeURIComponent(summonerName)}?api_key=${apiKey}`);
    res.json(response.data);
  } catch (error) {
...
app.get("/lol/league/v4/entries/by-summoner/:encryptedSummonerId", async (req, res) => {
  try {
    ...
    const response = await axios.get(`${baseURL}/lol/league/v4/entries/by-summoner/${encodeURIComponent(encryptedSummonerId)}?api_key=${apiKey}`);
    res.json(response.data);
  } catch (error) {
